...
U 2002. godini kada se pojavio Ruby je zauzimao 39. poziciju na TIOBE listi indeksa programskih jezika. Popularnost jezika se menjala kroz godine što možemo zaključiti sa slike \ref{fig:sajt3}, a trenutno se nalazi na 15. poziciji na TIOBE listi.
...
\begin{lstlisting}[language=Ruby, caption={Prvi primer koda}, frame=single, label=povVred]
...
\begin{lstlisting}[language=Ruby, caption={Prvi primer koda}, frame=single, label=imp]
	# imperativni nacin
	i = 0
...
\begin{lstlisting}[language=Ruby, caption={Prvi primer koda}, frame=single, label=mixins]
	class Primer
		include Comparable
		attr :str
		def <=>(other)
